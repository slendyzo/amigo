...
export default async function VehicleDetailPage({ params }: PageProps) {
  ...
  const asset = await prisma.realAsset.findFirst({
    where: { id, workspaceId: workspace.id, type: "VEHICLE" },
    include: {
      vehicle: true,
      liabilities: true,
  ...
  if (!asset || !asset.vehicle) notFound();
  ...
  return (
    <VehicleDetailClient
      ...
      vehicle={{
        ...
        trim: asset.vehicle.trim,
        generation: asset.vehicle.generation,
        mileage: asset.vehicle.mileage,
        plate: asset.vehicle.plate,
        color: asset.vehicle.color,
        fuelType: asset.vehicle.fuelType,
        bodyType: asset.vehicle.bodyType,
      }}
