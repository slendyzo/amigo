...
export function Modal({
  ...
  const panelProps = {
    ...
    className: cn(
      "modal-panel relative flex min-w-0 w-full flex-col overflow-hidden",
      SIZES[size],
      isSheet
        ? cn(
  ...
  // Escape transformed/overflow-clipped page ancestors. Context still flows
  // through the portal, including translations and nested form state.
  return createPortal(
    <div
      className={cn(
    ...
    <div className={cn("flex-shrink-0", className)}>
      ...
      {(title || showClose) && (
        <div className="flex items-center justify-between px-4 py-2 md:px-5 md:py-3">
          {typeof title === "string" ? (
            <h2 className="min-w-0 break-words text-[17px] leading-snug font-bold text-[var(--ink)]">{title}</h2>
          ) : (
            title
          )}
          {showClose && (
            <button
              ...
              onClick={onClose}
              aria-label="Close"
              className="tap-none flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[var(--ink-subtle)] transition-colors active:text-[var(--ink)] md:hover:text-[var(--ink)]"
...
export function ModalBody({ children, className }: ModalBodyProps) {
  ...
  return (
    <div
      ref={scrollerRef}
      className={cn(
        "modal-body scroll-touch min-h-0 min-w-0 flex-1 overflow-y-auto px-5 pb-4",
        className
      )}
    >
      {children}
