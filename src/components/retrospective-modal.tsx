...
function MomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-md border border-[var(--line)] bg-[var(--surface)] px-3 py-2 text-xs shadow-sm">
      <div className="text-[var(--ink-subtle)] mb-1">{payload[0]?.payload?.category}</div>
      <div className="flex gap-3">
        <span className="text-[var(--ink-subtle)]">Prev: {formatEur(payload[0]?.payload?.prevTotal ?? 0)}</span>
          ...
          >
            ...
            <div className="px-6 pt-5 pb-6 space-y-5">
              ...
              <motion.div
                ...
                className="rounded-lg border border-[var(--line)] divide-y divide-[var(--line)]"
              >
                {insight.content.observations.map((obs, i) => (
                  ...
                  >
                    <div className="text-[10px] font-semibold uppercase tracking-widest text-[var(--ink-subtle)] mb-1">
                      {indices[i]}
                    </div>
                ...
                <ResponsiveContainer width="100%" height={Math.max(140, chartData.length * 38)}>
                  ...
                  >
                    ...
                    <Bar dataKey="prevTotal" radius={[2, 2, 2, 2]}>
                      {chartData.map((_, idx) => (
                        <Cell key={idx} className="fill-[var(--accent-fainter)]" />
                      ))}
                    </Bar>
                    <Bar dataKey="currentTotal" radius={[2, 2, 2, 2]}>
                      {chartData.map((_, idx) => (
                        <Cell key={idx} className="fill-[var(--accent)]" />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
