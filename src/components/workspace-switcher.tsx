  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-[var(--surface-2)] transition-colors text-left"
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-medium text-[var(--ink)] truncate max-w-[120px]">
              {displayName}
            </div>
          </div>
        </div>
        {/* Chevron */}
        <svg
          className={`w-4 h-4 text-[var(--ink-subtle)] transition-transform ${isOpen ? "rotate-180" : ""}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute left-0 mt-1 w-64 bg-[var(--surface)] rounded-lg shadow-lg border border-[var(--line)] py-1 z-50">
          {isLoading ? (
            <div className="px-4 py-3 text-sm text-[var(--ink-subtle)] text-center">
              {t("loading") || "Loading..."}
            </div>
          ) : (
            <>
              ...
              <Link
                ...
                className="w-full px-4 py-2 flex items-center gap-2 text-sm text-[var(--ink-muted)] hover:bg-[var(--surface-2)] transition-colors"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
